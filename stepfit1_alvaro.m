function est=stepfit1_alvaro(data,varargin)
%    
%USAGE:
...
wb = waitbar(0,'Starting step fit function calculation...','Units','pixels','Visible','off');
movegui(wb,'center');
pos = get(wb,'Position');
movegui(wb,[pos(1),pos(2)-57]);
set(wb,'Visible','on');

for pass=1:passes
    
    waitbar(pass/passes,wb,['Calculating step fit function...',char(10),'Iteration ',num2str(pass),' of ',num2str(passes)]);
...
if(pass>1 ); %%sigmabar definition
    ...
    sigmabar=penaltyFactor*(noise.^2+measnoise^2+resolution^2);
end
...
for j=1:m
    ...
    if(nargin>4)
        if((strcmp(varargin{1},'normalized') || strcmp(varargin{1},'Normalized')) && sum(Nhist(:,j))~=0)
